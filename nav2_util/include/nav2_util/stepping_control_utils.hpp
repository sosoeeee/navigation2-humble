...
private:
    ...
    std::string id_;
    rclcpp_lifecycle::lifecycle_publisher<std_msgs::msg::String>::SharedPtr pub_;
public:
    SteppingPublisher(std::shared_ptr<rclcpp_lifecycle::LifecycleNode> node, std::string id);
    ~SteppingPublisher();
};

SteppingPublisher::SteppingPublisher(std::shared_ptr<rclcpp_lifecycle::LifecycleNode> node, std::string id)
: node_(node), id_(id)
{
...
SteppingPublisher::on_activate()
{
    pub_->on_activate();
}

SteppingPublisher::on_deactivate()
{
    pub_->on_deactivate();
}

SteppingPublisher::reset()
{
    pub_->reset();
}

SteppingPublisher::request_stop()
{
...
SteppingPublisher::request_restart()
{
    auto msg = std_msgs::msg::String();
...
SteppingPublisher::~SteppingPublisher()
{
}
